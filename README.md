...
- `cargo check`：通过；本轮新增批量 HTML 报告导出 command。
- `cargo test`：通过；37 个 Rust 单元测试覆盖 MP4 时长读取、无证据不生成 mock 问题、cropdetect 解析、上下/左右黑边风险覆盖、边缘黑像素占比兜底检测、全尺寸 cropdetect 结果过滤、兜底抽帧宽高比保持、桌面 `32.mp4` 黑边回归、黑边默认/自定义阈值、freezedetect 解析、按 FPS 的夹帧帧数分级、累计夹帧升红、桌面 `12.mp4` 夹帧回归、四角疑似 AI 标识启发式检测、AI 四角扫描区域设置、字幕 OCR 文本清洗、逐字匹配、重复 OCR 合并、字幕匹配错误处理、单文件/批量报告导出复用前端当前检测结果和 base64 编码。
