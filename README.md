...
- 日期：2026-05-27
- 阶段：MVP 初始化
- 状态：已完成基础工程、前端主界面、Tauri IPC、检测任务流、报告骨架、浏览器静态渲染验证，补齐 Tauri 默认图标，修复文件导入权限配置，为每个问题补齐开始/结束截图展示，接入 FFmpeg/ffprobe 真实基础信息读取、cropdetect 黑边检测、边缘黑像素占比黑边兜底检测、freezedetect 帧级夹帧/冻结帧检测、四角疑似 AI 标识检测和字幕 OCR/小说文本逐字匹配，新增黑边阈值与 AI 四角扫描位置设置，补强上下/左右黑边风险说明，修复主界面纵向滚动和报告导出结果一致性，并完成本机 FFmpeg/ffprobe 命令验证。
...
- `npm run build`：通过；本轮修复主界面纵向滚动、报告导出结果一致性和导出按钮可用状态。
- 浏览器静态渲染：通过，页面可显示工具栏、文件列表空态、结果面板空态和状态栏。
- `cargo check`：通过；本轮新增帧级夹帧/冻结帧检测。
- `cargo test`：通过；35 个 Rust 单元测试覆盖 MP4 时长读取、无证据不生成 mock 问题、cropdetect 解析、上下/左右黑边风险覆盖、边缘黑像素占比兜底检测、全尺寸 cropdetect 结果过滤、兜底抽帧宽高比保持、桌面 `32.mp4` 黑边回归、黑边默认/自定义阈值、freezedetect 解析、按 FPS 的夹帧帧数分级、累计夹帧升红、桌面 `12.mp4` 夹帧回归、四角疑似 AI 标识启发式检测、AI 四角扫描区域设置、字幕 OCR 文本清洗、逐字匹配、重复 OCR 合并、字幕匹配错误处理、报告导出复用前端当前检测结果和 base64 编码。
