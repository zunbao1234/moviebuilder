...
- 日期：2026-05-27
- 阶段：MVP 初始化
- 状态：已完成基础工程、前端主界面、Tauri IPC、检测任务流、报告骨架、浏览器静态渲染验证，补齐 Tauri 默认图标，修复文件导入权限配置，为每个问题补齐开始/结束截图展示，接入 FFmpeg/ffprobe 真实基础信息读取、cropdetect 黑边检测、边缘黑像素占比黑边兜底检测、freezedetect 帧级夹帧/冻结帧检测、四角疑似 AI 标识检测和字幕 OCR/小说文本逐字匹配，新增黑边阈值与 AI 四角扫描位置设置，补强上下/左右黑边风险说明，修复主界面纵向滚动和报告导出结果一致性，新增所有已完成文件的批量 HTML 报告导出，提升黑边检测稳定性，并完成本机 FFmpeg/ffprobe 命令验证。
- 检测说明：当前版本不再固定生成 1 红 / 2 黄 / 1 绿模拟问题；已接入真实视频基础信息、真实黑边检测、真实帧级夹帧/冻结帧检测、四角疑似 AI 标识检测和字幕 OCR/小说文本逐字匹配。跳帧、音画同步、音频和编码合规性仍待后续实现。
- 夹帧修复说明：旧实现按冻结持续秒数检测和分级，默认会过滤 1 秒以下片段，导致 30fps 视频中 1-5 帧的短夹帧漏检。当前实现按视频 FPS 把 FFmpeg `freezedetect` 阈值降到帧级，并按 PRD 的连续帧数/累计帧数规则分级。
...
- [x] 当 FFmpeg `cropdetect` 因边缘亮点/字幕/暗噪声返回全尺寸时，启用边缘黑像素占比兜底检测
- [x] 黑边兜底检测抽帧保持源视频宽高比，避免缩放补边造成误报
- [x] 黑边检测改为多时间点稳定投票，降低单个暗场/转场/字幕帧造成的漏检和误报
- [x] 使用桌面样片 `/Users/bey/Desktop/32.mp4` 验证黑边兜底检测可识别明显黑边
...
- [x] HTML 报告优先导出界面当前检测结果，避免重新检测造成报告信息缺失或不一致
- [x] 支持批量导出所有已完成检测文件的 HTML 报告
- [x] 优化结果详情面板布局，红黄绿分组在宽屏下并列展示，避免绿线提示被挤到不可见区域
- [x] 修复主界面内容超过窗口高度时无法纵向滚动的问题
...
- `npm run build`：通过；本轮新增“导出当前 / 批量导出”报告按钮和前端批量导出调用。
- 浏览器静态渲染：通过，页面可显示工具栏、文件列表空态、结果面板空态和状态栏。
- `cargo check`：通过；本轮新增黑边多抽样稳定聚合。
- `cargo test`：通过；39 个 Rust 单元测试覆盖 MP4 时长读取、无证据不生成 mock 问题、cropdetect 解析、上下/左右黑边风险覆盖、边缘黑像素占比兜底检测、黑边稳定投票、全尺寸 cropdetect 结果过滤、兜底抽帧宽高比保持、桌面 `32.mp4` 黑边回归、黑边默认/自定义阈值、freezedetect 解析、按 FPS 的夹帧帧数分级、累计夹帧升红、桌面 `12.mp4` 夹帧回归、四角疑似 AI 标识启发式检测、AI 四角扫描区域设置、字幕 OCR 文本清洗、逐字匹配、重复 OCR 合并、字幕匹配错误处理、单文件/批量报告导出复用前端当前检测结果和 base64 编码。
...
- 桌面验证视频 `/Users/bey/Desktop/32.mp4`：ffprobe 读到 74.58s、1080x1920、30fps、H.264；默认 cropdetect 返回全尺寸 `crop=1080:1920:0:0`，修复后通过边缘黑像素占比兜底检测生成黑边问题。
- 桌面验证视频 `/Users/bey/Desktop/12.mp4`：ffprobe 读到 73.51s、1080x1920、30fps、H.264；旧实现只检测 1-3 秒长冻结并过滤 1 秒以下片段，无法识别短夹帧；修复后 Balanced 模式按 2 帧起检，真实回归测试可生成 `夹帧/冻结帧` 红线问题。
